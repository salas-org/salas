...
# look for empty dir
KEY_STORE_DIR=$(pwd)/ethereum_data/keystore
if [ -d "$KEY_STORE_DIR" ]
then
	if [ "$(ls -A $KEY_STORE_DIR)" ]; then
     echo "keystore found"
	else
    echo "$KEY_STORE_DIR is Empty"
    echo "initializing miner"
    INIT_MINER='yes'
    ./2_init_miner.sh
	fi
else
  echo "Directory $KEY_STORE_DIR not found."
  echo "initializing miner"  
  INIT_MINER='yes'
  ./2_init_miner.sh
fi
...
miner_cmd="$SALAS_DIR/cmd/geth --bootnodes $SALAS_ENODES --nat none --datadir $MINER_ETH_DATA_PATH --syncmode full --verbosity $VERBOSITY \
