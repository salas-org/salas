...
then
	if [ "$(ls -A $MINER_KEYSTORE_PATH)" ]; then
     echo "keystore found"
	else
...
miner_cmd="$SALAS_DIR/cmd/geth --keystore $MINER_KEYSTORE_PATH --bootnodes $SALAS_ENODES --nat none --datadir $MINER_ETH_DATA_PATH --syncmode full --verbosity $VERBOSITY \
